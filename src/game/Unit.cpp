...
void Unit::Mount(uint32 mount, uint32 spellId)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
		...
		{
			Pet* pet = GetPet();
			if (pet!=nullptr&&!pet->isMercenary())//²»ÊÇ¹ÍÓ¶±ø²Åunsummon
        ...
        else if (SpellEntry const* spellInfo = sSpellStore.LookupEntry(spellId))
        {
            ...
            else if (Pet* pet = GetPet())
            {
                        ...
                        sWorld.getConfig(CONFIG_BOOL_PET_UNSUMMON_AT_MOUNT))
                {
					if (pet->isMercenary())
					{
						//pet->ApplyModeFlags(PET_MODE_DISABLE_ACTIONS, true);//¹ÍÓ¶±ø½ûÖ¹ÒÆ¶¯£¬Í¨¹ýmountÒÆ¶¯
						//return;
					}else
						((Player*)this)->UnsummonPetTemporaryIfAny();
                }
                //else
                    //pet->ApplyModeFlags(PET_MODE_DISABLE_ACTIONS, true);ÓÀÔ¶²»½ûÖ¹ACTIONBAR
            }
        }
...
void Unit::OnRelocated()
{
	...
	if (GetTypeId() == TYPEID_PLAYER && ((Player*)this)->context.isMovesplineRunning)
		return;//·ÉÐÐÖÐ£¬ËÙ¶ÈÌ«¿ì24.0f £¬ÎªÁË±£Ö¤¹ÍÓ¶±ø¾àÀë²»»áµ¼ÖÂunsumon
	else if (GetTypeId() == TYPEID_UNIT)
