...
bool Map::GetHitPosition(float srcX, float srcY, float srcZ, float& destX, float& destY, float& destZ, uint32 phasemask, float modifyDist) const
{
    ...
    if (result0)
    {
       //DEBUG_LOG("Map::GetHitPosition vmaps corrects gained with static objects! new dest coords are X:%f Y:%f Z:%f", destX, destY, destZ);//不跟踪撞击点
        destX = tempX;
        destY = tempY;
        destZ = tempZ;
    }
    ...
    if (result1)
    {
        DEBUG_LOG("Map::GetHitPosition vmaps corrects gained with dynamic objects! new dest coords are X:%f Y:%f Z:%f", destX, destY, destZ);//跟踪dynamic撞击点
        destX = tempX;
        destY = tempY;
        destZ = tempZ;
    }
