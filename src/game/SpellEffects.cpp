...
void Spell::EffectSchoolDMG(SpellEffectIndex effect_idx)
{
    if (unitTarget && unitTarget->isAlive())
    {
        ...
        //    m_damage += damage;//这里增加技能伤害的倍率逻辑
			if (m_caster->GetTypeId() == TYPEID_PLAYER)
				m_damage += damage * m_caster->ToPlayer()->context.mapDifficultyDamageMultiplier;
			else if (m_caster->GetUInt32Value(OBJECT_FIELD_ENTRY) == 70000)//雇佣兵
				m_damage += damage * m_caster->GetOwner()->ToPlayer()->context.mapDifficultyDamageMultiplier;
			else
...
void Spell::EffectWeaponDmg(SpellEffectIndex eff_idx)
{
	...
	if (bonus >= 0){
		if (m_caster->GetTypeId() == TYPEID_PLAYER)//玩家
			m_damage += bonus * m_caster->ToPlayer()->context.mapDifficultyDamageMultiplier;
		else if (m_caster->GetUInt32Value(OBJECT_FIELD_ENTRY) == 70000)//雇佣兵
			m_damage += bonus  * m_caster->GetOwner()->ToPlayer()->context.mapDifficultyDamageMultiplier;
		else
