...
void Player::RegenerateHealth(uint32 diff)
{
	...
	addvalue *= context.mapDifficultyHealthMultiplier; /*回血速度随副本难度动态调整*/
...
void Player::GiveXP(uint32 xp, Unit* victim)
{
	...
	xp *= 1.0f / context.mapDifficultyDamageMultiplier;//获得经验必须做倍率减少，否则会出现经验40倍，都去副本练级了
	...
	if (pet->IsPermanentPetFor(this)&&!pet->isMercenary())//Mercenary的技能另外添加，此处只提供基础技能
    {
        ...
        for (PetSpellMap::const_iterator itr = pet->m_spells.begin(); itr != pet->m_spells.end(); ++itr)
        {
            if (itr->second.state == PETSPELL_REMOVED)
                continue;
			DEBUG_LOG("%d=%d", itr->first, itr->second.active);
...
inline void BeforeVisibilityDestroy<Creature>(Creature* t, Player* p)
{
	if (p->GetPetGuid() == t->GetObjectGuid() && ((Creature*)t)->IsPet())
		p->UnsummonPetTemporaryIfAny();//改为临时Unsummon
