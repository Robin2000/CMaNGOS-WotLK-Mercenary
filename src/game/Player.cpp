...
void Player::SendQuestUpdate(uint32 questId)
{
	...
	SpellAreaForAreaMapBounds saBounds = sSpellMgr.GetSpellAreaForQuestMapBounds(questId);
...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder* holder)
{
	...
	// 积分
	context.gamePointMgr._LoadAccountBalance(holder->GetResult(PLAYER_LOGIN_QUERY_LOADGACCOUNTBALANCE));
	context.gamePointMgr._LoadCharacterExt(holder->GetResult(PLAYER_LOGIN_QUERY_CHARACTEREXT));
	//雇佣兵
	context.loadMercenary();
    // Mail
...
void Player::SaveToDB()
{
    ...
    _SaveGlyphs();
    _SaveTalents();
	context.gamePointMgr._SaveGamePoint();
...
void Player::changeCamera(float x, float y, float z, float orientation,int32 duration, float radius){
...
void Player::changeCamera(float x, float y, float orientation, int32 duration, float radius){
	TerrainInfo const* map = sTerrainMgr.LoadTerrain(GetMapId());
	float z = map->GetWaterOrGroundLevel(x, y, MAX_HEIGHT);
	changeCamera( x,  y, z, orientation,  duration,  radius);
}
