...
AccountOpResult AccountMgr::CreateAccount(std::string username, std::string password, uint32 expansion, std::string email)
{
	if (utf8length(username) > MAX_ACCOUNT_STR)
		return AOR_NAME_TOO_LONG;                           // username's too long
	...
	if (!LoginDatabase.PExecute("INSERT INTO account(username,sha_pass_hash,joindate,expansion,email) VALUES('%s','%s',NOW(),'%u','%s')", username.c_str(), CalculateShaPassHash(username, password).c_str(), expansion, email))
		return AOR_DB_INTERNAL_ERROR;                       // unexpected error
	LoginDatabase.Execute("INSERT INTO realmcharacters (realmid, acctid, numchars) SELECT realmlist.id, account.id, 0 FROM realmlist,account LEFT JOIN realmcharacters ON acctid=account.id WHERE acctid IS NULL");
